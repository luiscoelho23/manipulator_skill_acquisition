...
def save_model_on_exit():
    ...
    try:
        ...
        model_path = os.path.join(current_output_config['directory'], 
                            f"ddpg_{current_output_config['model_name']}_{current_date}_interrupted.pt")
...
class DDPG_Agent:
    def __init__(self, state_dim=7, action_dim=3, learning_rate=0.0001, gamma=0.99, 
        ...
        self.buffer_limit = int(buffer_limit)  # Make sure buffer_limit is an integer
        self.tau = tau
        ...
        # Initialize Actor (Policy) and Critic
    ...
    def choose_action(self, s, eval_mode=False):
        with torch.no_grad():
            ...
            # DDPG uses deterministic policy (no sampling)
            action = self.actor(state)
            
            # Add exploration noise during training if not in eval mode
            if not eval_mode:
    ...
    def train_agent(self):
        ...
        for _ in range(self.gradient_steps):
            ...
            # Compute critic loss
            with torch.no_grad():
                # Use deterministic target policy (no sampling)
                next_actions = self.actor_target(s_prime_batch)
                target_q = self.critic_target(s_prime_batch, next_actions)
            ...
            critic_loss = F.smooth_l1_loss(current_q, target, reduction='none')
            ...
            # Only update actor periodically to stabilize training
            if self.training_step % 2 == 0:
                # Compute actor loss using deterministic policy (no sampling)
                actions = self.actor(s_batch)
                actor_loss = -self.critic(s_batch, actions).mean()
...
def train(config_path):
    ...
    if not hasattr(env, 'observation_space'):
        ...
        state_dim = 7  # Default from SAC agent
    ...
    if not hasattr(env, 'action_space'):
        ...
        action_dim = 3  # Default from SAC agent
    ...
    for EP in range(EPISODE):
        ...
        while not done:
            ...
            # Remove the separate exploration noise addition here since it's now handled in choose_action
            # Just clip to action space bounds
        ...
        print("EP:{}, Score:{:.1f}, Noise:{:.4f}".format(EP, score, current_noise))
            ...
            if eval_score > best_eval_score:
                ...
                if output_config.get('save_best', True):
                    model_path = os.path.join(output_config['directory'], 
                                            f"ddpg_{output_config['model_name']}_{current_date}_best.pt")
        ...
        if output_config.get('save_checkpoints', True) and EP % output_config.get('save_frequency', 100) == 0:
            model_path = os.path.join(output_config['directory'], 
                                    f"ddpg_{output_config['model_name']}_{current_date}_EP{EP}.pt")
            torch.save(agent.actor.state_dict(), model_path)
...
if __name__ == '__main__':
    if len(sys.argv) != 2:
        print("Usage: python train_ddpg.py <config_path>")
        sys.exit(1)
